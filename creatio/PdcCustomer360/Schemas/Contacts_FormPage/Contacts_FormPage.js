define("Contacts_FormPage", /**SCHEMA_DEPS*/ [] /**SCHEMA_DEPS*/, function () /**SCHEMA_ARGS*/ /**SCHEMA_ARGS*/ {
  return {
    viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/ [
      {
        operation: "insert",
        name: "TabContainer_yt0qdvt",
        values: {
          type: "crt.TabContainer",
          items: [],
          caption: "#ResourceString(TabContainer_yt0qdvt_caption)#",
          iconPosition: "only-text",
          visible: true,
        },
        parentName: "Tabs",
        propertyName: "items",
        index: 6,
      },
      {
        operation: "insert",
        name: "GridContainer_zaupkf0",
        values: {
          type: "crt.GridContainer",
          items: [],
          rows: "minmax(32px, max-content)",
          columns: ["minmax(32px, 1fr)", "minmax(32px, 1fr)"],
          gap: {
            columnGap: "large",
            rowGap: 0,
          },
        },
        parentName: "TabContainer_yt0qdvt",
        propertyName: "items",
        index: 0,
      },
      {
        operation: "insert",
        name: "PandaDoc_x95wi5u",
        values: {
          layoutConfig: {
            column: 1,
            row: 1,
            colSpan: 2,
            rowSpan: 5,
          },
          type: "pdc.PandaDoc",
          width: "100%",
          height: "70vh",
          entityType: "contacts",
          entityId: "$Id",
          providerName: "creatio",
          externalEditor: true,
          isExternalRunner: true,
        },
        parentName: "GridContainer_zaupkf0",
        propertyName: "items",
        index: 0,
      },
    ] /**SCHEMA_VIEW_CONFIG_DIFF*/,
    viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/ [
    ] /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/,
    modelConfigDiff:
      /**SCHEMA_MODEL_CONFIG_DIFF*/ [] /**SCHEMA_MODEL_CONFIG_DIFF*/,
    handlers: /**SCHEMA_HANDLERS*/ [] /**SCHEMA_HANDLERS*/,
    converters: /**SCHEMA_CONVERTERS*/ {} /**SCHEMA_CONVERTERS*/,
    validators: /**SCHEMA_VALIDATORS*/ {} /**SCHEMA_VALIDATORS*/,
  };
});
